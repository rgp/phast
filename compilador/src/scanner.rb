    ...
    def next_token
        ...
        until !token.empty?
            ...
            when @scanner.scan(/\?>/)
                ...
                #KEYWORDS
            when match = @scanner.scan(/int\b/i)
                token = [:WORD_INT, match]
            when match = @scanner.scan(/float\b/i)
                token = [:WORD_FLOAT, match]
            when match = @scanner.scan(/if\b/i)
                token = [:WORD_IF, match]
            when match = @scanner.scan(/else\b/i)
                token = [:WORD_ELSE, match]
            when match = @scanner.scan(/while\b/i)
                token = [:WORD_WHILE, match]
            when match = @scanner.scan(/for\b/i)
                token = [:WORD_FOR, match]
            when match = @scanner.scan(/class/i)
                token = [:WORD_CLASS, match]
            when match = @scanner.scan(/extends\b/i)
                token = [:WORD_EXTENDS, match]
            when match = @scanner.scan(/implements\b/i)
                token = [:WORD_IMPLEMENTS, match]
            when match = @scanner.scan(/do\b/i)
                token = [:WORD_DO, match]
            when match = @scanner.scan(/switch\b/i)
                token = [:WORD_SWITCH, match]
            when match = @scanner.scan(/case\b/i)
                token = [:WORD_CASE, match]
            when match = @scanner.scan(/break\b/i)
                token = [:WORD_BREAK, match]
            when match = @scanner.scan(/default\b/i)
                token = [:WORD_DEFAULT, match]
            when match = @scanner.scan(/continue\b/i)
                token = [:WORD_CONTINUE, match]
            when match = @scanner.scan(/fun\b/i)
                token = [:WORD_FUN, match]
            when match = @scanner.scan(/mem\b/i)
                token = [:WORD_MEM, match]
            when match = @scanner.scan(/return\b/i)
                token = [:WORD_RETURN, match]
            when match = @scanner.scan(/static\b/i)
                token = [:WORD_STATIC, match]
            when match = @scanner.scan(/abstract\b/i)
                token = [:WORD_ABSTRACT, match]
            when match = @scanner.scan(/public\b/i)
                token = [:WORD_PUBLIC, match]
            when match = @scanner.scan(/private\b/i)
                token = [:WORD_PRIVATE, match]
            when match = @scanner.scan(/protected\b/i)
            ...
            when match = @scanner.scan(/new\b/i)
            ...
            when match = @scanner.scan(/try\b/i)
                token = [:WORD_TRY, match]
            when match = @scanner.scan(/catch\b/i)
                token = [:WORD_CATCH, match]
            when match = @scanner.scan(/throw\b/i)
